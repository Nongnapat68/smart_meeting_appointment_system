...
export async function generateMeetingSummary(input: MeetingSummaryInput): Promise<string> {
  ...
  const response = await anthropic.messages.create({
    ...
    max_tokens: 1200,
    system:
      "คุณเป็นผู้ช่วยเตรียมข้อมูลก่อนการประชุมสำหรับระบบ Smart Meeting ของสถาบันการศึกษา " +
...
export async function generatePendingIssuesAnalysis(input: PendingIssuesInput): Promise<string> {
  ...
  const response = await anthropic.messages.create({
    ...
    max_tokens: 1200,
    system:
      "คุณเป็นผู้ช่วยวิเคราะห์ประเด็นค้างก่อนการประชุมครั้งถัดไปสำหรับระบบ Smart Meeting ของสถาบันการศึกษา " +
...
export async function generateAgendaSuggestion(input: AgendaSuggestionInput): Promise<string> {
  ...
  const response = await anthropic.messages.create({
    ...
    max_tokens: 1200,
    system:
      "คุณเป็นผู้ช่วยร่างวาระ (agenda) การประชุมครั้งถัดไปสำหรับระบบ Smart Meeting ของสถาบันการศึกษา " +
