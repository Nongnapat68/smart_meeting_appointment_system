...
| `update_meeting_with_participants(p_meeting_id text, p_title text, p_start_time timestamp, p_end_time timestamp, p_description text, p_type text, p_status text, p_location text, p_project_id text, p_online_meeting_resource_id text, p_participant_person_ids text[], p_group_ids text[], p_external_emails text[])` | Hybrid migration รอบ Meeting — edit (แทน `PUT /api/meetings/[id]` เดิม, **มีใช้งานจริงแล้ว** ผ่าน `MeetingForm.tsx`) | `"Meeting"` | INVOKER | Partial update field ธรรมดาของ `Meeting` (plain column SET ทุกครั้ง — caller จริงส่งครบทุก field เสมอ ไม่มีเคส "ไม่ส่งมาให้คงค่าเดิม" ที่ต้องรองรับ) จากนั้น**ถ้า** `p_group_ids IS NOT NULL` ค่อย replace `_MeetingGroups` ทั้งชุด และ**ถ้า**พารามิเตอร์ participant-related ตัวใดตัวหนึ่ง (`p_participant_person_ids`/`p_group_ids`/`p_external_emails`) `IS NOT NULL` ค่อย delete+resolve `MeetingParticipant` ใหม่ทั้งชุดด้วยตรรกะ DIRECT/GROUP/EXTERNAL เดียวกับ `create_meeting_with_participants()` เป๊ะ (รวม organizer บังคับ ORGANIZER/DIRECT เสมอ) — `NULL` บนพารามิเตอร์เหล่านี้แปลว่า "ไม่แตะ" ต่างจาก array ว่าง (`[]`) ที่แปลว่า "ลบทั้งหมด" ทั้งหมดในฟังก์ชันเดียว (rollback อัตโนมัติถ้าล้มเหลวจุดใดจุดหนึ่ง รวมถึง field ธรรมดาที่ update ไปแล้วก่อนหน้าด้วย) ไม่แตะ `Reminder`/`Notification` เลยเหมือน handler เดิม เป็น `SECURITY INVOKER` เพราะไม่มี insert ใดที่ผู้เรียกไม่มีสิทธิ์ทำเอง — `update_organizer_or_admin` ของ `Meeting` และ `insert/delete_meeting_organizer_or_admin` ของทั้ง `MeetingParticipant`/`_MeetingGroups` คุมสิทธิ์ให้ครบอยู่แล้ว มีเช็คสิทธิ์ตรงๆ ที่บรรทัดแรกกับ `organizerId` **เดิม**ของ meeting (ไม่ใช่พารามิเตอร์ — update เปลี่ยนตัวผู้จัดไม่ได้) เพื่อให้ error message ตรงกับ `assertOwner()` เดิม ดู `docs/DESIGN_DECISIONS.md` §5.7 (รวม edge case ที่บันทึกไว้ตรงๆ: external email ที่ชนกับ Person ภายในของคนอื่นอาจขึ้น error แทนที่จะแนบเงียบๆ แบบตอน DEFINER ของ create) |

`process_due_reminders()`/`get_meeting_context()` เป็น `SECURITY INVOKER` เพราะแค่ query ข้อมูล ไม่ต้องข้าม RLS ของใคร รันในสิทธิ์ผู้เรียกตามปกติ — เช่นเดียวกับ `update_project_with_members()`/`update_meeting_with_participants()` (RLS ของ `Project`/`Meeting` เองบล็อกให้อยู่แล้ว ไม่มี insert ใดที่ต้อง bypass) ส่วน `create_meeting_with_participants()` เป็น `DEFINER` แบบเดียวกับ `is_admin()`/`is_meeting_participant()` ที่ RLS policies พึ่งพา เพราะต้อง insert แถว `Notification` ที่ผู้เรียกเองไม่มีสิทธิ์ insert ตรงๆ (ดูเหตุผลแยกแต่ละตัวในคอลัมน์ Logic ด้านบน)
